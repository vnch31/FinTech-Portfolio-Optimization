...
class GRU(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, dropout_prob, lower_bound=0.1, upper_bound=0.5):
    ...
    def forward(self, x):
        ...
        # rebalance weights
        out = torch.stack(
            [self.rebalance(weights, self.lower_bound, self.upper_bound) for weights in out])
